...
namespace Raytracer
{
	...
	inline Vec3f CastRay(const Vec3f& Origin, const Vec3f& Direction, const std::vector<Sphere>& spheres, const std::vector<Light>& lights, size_t Depth = 0)
	{
		...
		{
			std::int_fast32_t x{ (std::max)(0, (std::min)(SphericalMapWidth - 1, static_cast<std::int_fast32_t>((std::atan2f(Direction.z, Direction.x) / (lwmf::DoublePI) + 0.5F) * SphericalMapWidth))) };
	...
	inline void Draw()
	{
		static const Material Glass(1.5, Vec4f(0.0, 0.5, 0.1, 0.8), Vec3f(0.6, 0.7, 0.8), 125.);
