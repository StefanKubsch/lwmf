...
namespace Copperbars
{
	...
	inline void DrawCopperbar(std::int_fast32_t y, const std::int_fast32_t ColorAdd)
	{
		...
		{
			lwmf::Line(Canvas, 0, y, Canvas.Width - 1, y, lwmf::RGBAtoINT(CopperList[i + ColorAdd].Red, CopperList[i + ColorAdd].Green, CopperList[i + ColorAdd].Blue, 255));
		}
	}
	...
	inline void Draw()
	{
		...
		{
			DrawCopperbar(Canvas.HeightMid + SinTable[BlueBars[i]], 31);
			BlueBars[i] = (BlueBars[i] + Speed) % 360;
		}

		for (std::int_fast32_t i{ 3 }; i >= 0; --i)
		{
			DrawCopperbar(Canvas.HeightMid + SinTable[GreenBars[i]], 16);
			GreenBars[i] = (GreenBars[i] + Speed) % 360;
		}

		for (std::int_fast32_t i{ 3 }; i >= 0; --i)
		{
			DrawCopperbar(Canvas.HeightMid + SinTable[RedBars[i]], 1);
